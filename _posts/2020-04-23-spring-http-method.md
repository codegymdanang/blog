...
Thông thường thì từ client gửi yêu cầu lên server sẽ có nhiều loại yêu cầu. Có khi là client yêu cầu danh sách các sản phẩm, có khi là yêu cầu thêm một sản phẩm, hoặc xoá một sản phẩm. Tuỳ vào mong muốn của client mà trong lập trình viên chúng ta có nhiều loại yêu cầu hỗ trợ khác nhau.
...
Put là để cập nhật thông tin và Delete dùng để xóa thông tin.
...
1. GET: được sử dụng để lấy thông tin từ server theo URL đã cung cấp.
2. HEAD: giống với GET nhưng response trả về không có body, chỉ có header.
3. POST: gửi thông tin tới server thông qua các biểu mẫu http( ví dụ như đăng ký..).
...
-IDEMPOTENT : các method được coi là idempotent khi nó có thể thực hiện n + 1 lần mà vẫn trả lại 1 kết quả như ban đầu.
...
Một chút khác biệt dễ nhận thấy giữa <b>phương thức get</b> và <b>phương thức post</b> là get thì không có body. Khi dùng get để truyền dữ liệu lên server chúng ta thấy rằng tất cả các parameter đều bị hiển thị trên url của request, xét về khía cạnh bảo mật thì điều này thật là tệ.
...
Post thì khác, nó giấu parameters trong body và mã hóa chúng đi, ngăn cản các phần tử trung gian ăn cắp nội dung. Nhưng post chỉ có tính an toàn đối với client, còn với server thì lại khác.
...
- Điểm khác biệt giữa <b>phương thức post</b> và <b>phương thức put</b> đơn giản là put là idempotent còn post thì không, bạn sẽ nhận được thông báo lỗi khi gửi một request post với cùng 1 nội dung 2 lần nhưng put thì không, nó luôn trả về kết quả như nhau.
- Post: tạo mới.
