...
description : Giới thiệu và hướng dẫn cụ thể giúp các bạn sử dụng được 3 loại danh sách list trong lập trình web HTML bao gồm danh sách không sắp xếp, sắp xếp và danh sách giống từ điển. Chỉ ra các thẻ được sử dụng để tạo ra 3 loại danh sách trên như ul, ol, dl, dt, dd và các thuộc tính như circle, disc, type, style. Với các thẻ tạo danh sách list trong HTML này sẽ giúp các bạn tạo ra những danh sách list phù hợp cho mỗi trang web khác nhau.  
